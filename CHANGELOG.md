# Changelog

## Unreleased

## 1.1.2

- Ported to Minecraft 26.3 (Fabric Loader, Fabric API, and Xaero's World Map/Minimap/Lib bumped to their 26.3 releases).
- Fixed `GpuTextureView` import path, which moved to `com.mojang.renderpearl.api.textures` in 26.3.
- Added a toggle to view only selected biomes in the Biome Filter menu.
- Improved menu performance by caching the active biome list while the screen is open.
- Fixed optional Minimap filtering using Xaero's `setRGB` arguments in the wrong order.
- Refreshes existing Minimap tiles when filter settings change.
- Updated the development toolchain and Xaero dependencies for Minecraft 26.2.
- Added config persistence warnings and localized the page indicator.

## 1.1.1

- Fixed the biome filter toolbar button being hidden by Xaero's 26.2 zoom buttons.

## 1.1.0

- Updated to Minecraft 26.2.
- Updated Xaero's World Map support to 1.42.0.
- Updated optional Xaero's Minimap support to 26.2.0.

## 1.0.0

- Initial release for Minecraft 26.1.2.
